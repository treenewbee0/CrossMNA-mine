...
class Graph(defaultdict):
    ...
    def negative_sampling(self,K,a,b,layers,layid):
        # return [self.index2node[self.node_sampling.sampling()] for _ in range(K)]
        uj_layer=[]
        for _ in range(K):
            node=self.node_sampling.sampling()
            while node==b or node in layers[layid][a]:
                node=self.node_sampling.sampling()
            if node in layers[layid].keys():
                uj_layer.append([node,layid])
            else:
                layer_has_this_node=[]
                for layerid in layers:
                    if layerid==layid:continue
                    if node in layers[layerid].keys():
                        layer_has_this_node.append(layerid)
                uj_layer.append(([node,random.choice(layer_has_this_node)]))
        # print(uj_layer)
        return uj_layer
...
def gen_batches(layers,total_layer, batch_size, K=1):               #
    """
    ...
    for layid in layers:
        ...
        for a, b, w in edges:
            ...
            label = [1] + [-1] * K
            true_layer = [layid] * (K + 1)

            uj_layer=total_layer.negative_sampling(K,a,b,layers,layid)
            u_j=[b]
            for uj in uj_layer:
                u_j.append(uj[0])
            ...
            this_layer=[layid]
            for lay in uj_layer:
                this_layer.append(lay[1])
            # print(u_j,this_layer)


            node_samples.append([u_i, u_j, label, true_layer,this_layer])          #node_sample里一个正例一个负例
    ...
    node_samples += node_samples[:num_left]                     #为啥加这一部分？

    # split samples into batches
    samples = []
    for i in range(len(node_samples) // batch_size):
        batch = node_samples[i * batch_size: (i + 1) * batch_size]
        u_i, u_j, label, true_layer,this_layer = zip(*batch)                       #zip函数 从参数中的多个迭代器取元素组合成一个新的迭代器

        u_i = np.concatenate(u_i)                                   #concatenate拼接
        u_j = np.concatenate(u_j)

        label = np.concatenate(label)
        true_layer=np.concatenate(true_layer)
        this_layer = np.concatenate(this_layer)
        samples.append([u_i, u_j, label, true_layer,this_layer])               #sample里的数据形式是以每一批里ui列表，uj列表，label列表，thislayer列表依次存的    zip生成新的迭代器把ui，uj这些转置表示
...
def readfile(graph_path=None):
    ...
    graphs = pd.read_csv(graph_path, sep=' ', header=None,engine = "python")
    ...
    # map node to id                                                                重新标号的规则 从头开始连续的标号  keepnodes也在其中
    ...
    for layerID, graph in graphs.groupby(['layerID']):
        ...
        for a, b, w in edges:
            ...
            total_edges.append([a, b, w])
        layers[layerID] = load_edgelist(new_edges)
    total_layer = load_edgelist(total_edges)
    #能否对所有网络进行聚合形成一个大网络，在大网络中进行全局负采样




    num_nodes = max((id2node.keys()))+1
    return layers, num_nodes, id2node,total_layer                                     #每一层变成一个Graph对象，重新标号


def get_alignment_emb(inter_vectors, layers, id2node):                      #返回keepnodes标号不变（经过一次标号改变的）的对应的emb
