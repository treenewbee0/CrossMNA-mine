...
"""step 1. load data"""
layers, num_nodes, id2node,total_layer = readfile(graph_path=path)
num_layers = len(layers.keys())
# print (total_layer.keys())

"""step 2. initial negative sampling table"""
for layerid in layers:
    g = layers[layerid]
    g.init_neg()
total_layer.init_neg()
...
with tf.Session(config=config) as sess:
    ...
    for epoch in range(22):
        t = time.clock()
        batches = gen_batches(layers,total_layer, batch_size=args.batch_size, K=1)                                  #得到sample：所有的正例+K倍负例
        print("epoch {0}: time for generate batches={1}s".format(epoch, time.clock()-t))

        total_loss = 0.0
        t = time.clock()
        for batch in batches:
            u_i, u_j, label, true_layer,this_layer = batch
            feed_dict = {model.u_i: u_i, model.u_j: u_j, model.label: label,model.true_layer:true_layer,
        ...
        if epoch  ==  21:
            if args.task == 'NetworkAlignment':
                ...
                result = eval_emb('node_matching/' + args.dataset + '/networks' + str(args.p) + '.pk', node2vec, )                  #path里存的是newnetwork：（全部边都在newnetwork里，非keepnodes点都改成新id了）
