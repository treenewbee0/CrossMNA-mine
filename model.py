...
class MultiNetworkEmb(object):
    def __init__(self, num_of_nodes, batch_size, K, node_embedding,
        ...
        self.true_layer = tf.placeholder(name='true_layer', dtype=tf.int32, shape=[batch_size * (K + 1)])
        ...
        def f1():
            ...
            # step 3. layer emb r
            self.l_i_embedding = tf.nn.embedding_lookup(self.L_embedding,self.true_layer)
            self.l_j_embedding = tf.nn.embedding_lookup(self.L_embedding, self.this_layer)

            # step 4. r_i = u_i*W + l
            self.r_i = self.u_i_embedding + self.l_i_embedding
            self.r_j = self.u_j_embedding + self.l_j_embedding
